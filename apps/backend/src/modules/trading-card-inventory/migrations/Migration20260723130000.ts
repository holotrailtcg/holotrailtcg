...
/**
 * Stage 1 (import identity & review corrections), requires-separate-listing
 * review override: widens `trading_card_inventory_audit_entry`'s action
 * check constraint for the new `PROPOSAL_SEPARATE_LISTING_OVERRIDDEN`
 * action. Purely additive.
 */
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table "trading_card_inventory_audit_entry" add constraint "trading_card_inventory_audit_entry_action_check"
      check (action in (
        ...
        'PROPOSAL_SPLIT', 'ENTRY_MATCH_REMATCHED', 'PROPOSAL_SEPARATE_LISTING_OVERRIDDEN'
      ));`)
  }

  override async down(): Promise<void> {
    this.addSql(`delete from "trading_card_inventory_audit_entry" where action = 'PROPOSAL_SEPARATE_LISTING_OVERRIDDEN';`)
    this.addSql(`alter table "trading_card_inventory_audit_entry" drop constraint if exists "trading_card_inventory_audit_entry_action_check";`)
    this.addSql(`alter table "trading_card_inventory_audit_entry" add constraint "trading_card_inventory_audit_entry_action_check"
      check (action in (
        'SOURCE_CREATED', 'SOURCE_RENAMED', 'SOURCE_ARCHIVED', 'SOURCE_RESTORED',
